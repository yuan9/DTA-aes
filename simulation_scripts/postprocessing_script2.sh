...
echo "${plain_text/Initialplaintext/}${cipher_text/Finalciphertext/}">>$final_dir/inFiles_wddl_newddc/plain_cipher_sync.csv
...
mkdir $final_dir/outFiles_wddl_newddc/

for ((i=0; i<$numtrace; i+=1));
do
  a=`printf "%03d" $i`
  python outfile_postprocess_ptpx.py $final_dir/inFiles_wddl_newddc/yuan_power_$i.out $final_dir/outFiles_wddl_newddc/vector_$a.out $design 5 6
  echo "Trace-$i for design $design"
done
 # copy the plaintext and ciphertext file from the infiles folder to the outfiles folder
cp $final_dir/inFiles_wddl_newddc/plain_cipher_sync.csv $final_dir/outFiles_wddl_newddc/
