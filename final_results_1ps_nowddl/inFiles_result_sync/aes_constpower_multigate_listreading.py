...
rank= 3
fp="final_ACA_TVLA_phase2-2020-03-10-09-38.txt"
...
with open(fp) as f:
    for i,line in enumerate(f):
        if i < rank:
	   mod = line.split(' ')[0]
	   modname_list.append(mod)
print(modname_list)	


with open('yuan_power_99.out') as f_in:            
#f_in  = file('yuan_power_66.out', 'r')
    for modname in modname_list: 
	 ...
	 print(modname2)
         ...
         for i, line in enumerate(f_in):
             if i in range(8584, 8586): 
                w = line.split(' ')
		print w[1]
    	    #print(f_in)
                if w[0] == '.index':
            	    if w[1] == modname2 and w[3] == 'Pc\n':
                            id = int(w[2])
            	            idlist.append(id)
                            print 'find id %d' %(id)
                            break
